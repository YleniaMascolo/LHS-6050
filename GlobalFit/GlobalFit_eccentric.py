...
import emcee_run
import stats_eccentric as stats



"Global fit script for system LHS 6050 assuming an eccentric orbit. Joint analisys of photometric (TESS and ground based) and ESPRESSO datasets"
...
if __name__ == "__main__":
    ...
    chain_file = os.path.join(base_dir, "retrieval_chain_eccentric.h5")        
    ...
    labels = ["$t_0$", "P", "$R_p/R_s$", "b", "h", "k", "mstar", "rstar", "q1_tess", "q2_tess", "q1_lco", "q2_lco", "q1_lcog", "q2_lcog", "q1_minerva", "q2_minerva", "q1_ngts", "q2_ngts", "q1_trap", "q2_trap",
    ...
    chain_plot = os.path.join(base_dir, "chain_plot_eccentric.pdf")
    ...
    plt.tight_layout()
    corner_plot = os.path.join(base_dir, "corner_plot_eccentric.pdf")
    plt.savefig(corner_plot)
    plt.show()

    mcmc_results = os.path.join(base_dir, "mcmc_results_eccentric.npz")
    ...
    # Best-fit parameters
    # -----------------------------
    labels = ["t_0", "P", "R_p/R_s", "b", "h", "k", "mstar", "rstar", "q1_{tess}", "q2_{tess}", "q1_{lco}", "q2_{lco}", "q1_{lcog}", "q2_{lcog}", "q1_{minerva}", "q2_{minerva}", "q1_{ngts}", "q2_{ngts}", "q1_{trap}", "q2_{trap}",
    ...
    # -----------------------------
    t0, per, rp, b, h, k, mstar, rstar, _, _, _, _, _, _, _, _, _, _, _, _, krv, *_= samples.T
    ...
    Mp = (krv * (np.sqrt(1-(e**2))) * (M_star**(2/3)) * ((per_sec / (2*np.pi*G))**(1/3))) / sini
    ...
    best_fit_results = os.path.join(base_dir, "best_fit_results_eccentric.json")
