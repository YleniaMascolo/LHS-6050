...
""" Models: transit and RV assuming a circular orbit"""

# -----------------------------
#  Quadratic limb darkening function (Kipping 2013)
# -----------------------------
def limb_darkening(q1, q2):
    """Converts Kipping (2013) efficient sampling parameters to quadratic limb darkening coefficients.
    """
    sqrt_q1 = np.sqrt(q1)
    u1 = 2 * sqrt_q1 * q2
    u2 = sqrt_q1 * (1 - 2 * q2)
...
def inverse_limb_darkening(u1, u2, err_u1, err_u2):
    """Transforms quadratic limb darkening coefficients back to Kipping (2013) parameters.
    """
    q1 = (u1 + u2)**2
    q2 = 0.5 * u1 / (u1 + u2)
    
    # First-order analytical error propagation
...
def planetmodel(theta, t):
    ...
    # Calculate the orbital phase relative to the reference transit time
    phase = (t - t0) / per
