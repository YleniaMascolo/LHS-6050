...
a circular orbit. It calculates boundary conditions through a log-prior, multi-instrument 
log-likelihoods, and combines them into the final log-posterior function. 
It provides two separate stellar log-likelihood variants to allow the user to easily 
choose between Uniform or Gaussian priors on the Kipping limb darkening parameters.
"""
...
# -----------Stellar parameters and Limb darkening from LDTK------------- 
mstar_obs = 0.224
rstar_obs = 0.254
mstar_obs_err = 0.007
rstar_obs_err = 0.013
...
# ---- TESS ----
u1_tess, u2_tess = 0.26276141, 0.36392092
err_u1_tess, err_u2_tess = 0.00195121, 0.00341829

q1_tess_ldtk, q2_tess_ldtk, err_q1_tess_ldtk, err_q2_tess_ldtk = models.inverse_limb_darkening(u1_tess, u2_tess, err_u1_tess, err_u2_tess)

# ---- LCO (i-band) ----
u1_lco, u2_lco = 0.31562241, 0.4057571
err_u1_lco, err_u2_lco = 0.00320627, 0.00522504

q1_lco_ldtk, q2_lco_ldtk, err_q1_lco_ldtk, err_q2_lco_ldtk = models.inverse_limb_darkening(u1_lco, u2_lco, err_u1_lco, err_u2_lco)

# ---- LCO (g-band) ----
u1_lcog, u2_lcog = 0.51421344, 0.37492077
err_u1_lcog, err_u2_lcog = 0.00372648, 0.00500886

q1_lcog_ldtk, q2_lcog_ldtk, err_q1_lcog_ldtk, err_q2_lcog_ldtk = models.inverse_limb_darkening(u1_lcog, u2_lcog, err_u1_lcog, err_u2_lcog)

# ---- MINERVA  (350-975 nm)----
u1_minerva, u2_minerva = 0.28992322, 0.37205758
err_u1_minerva, err_u2_minerva = 0.00208266, 0.00345203

q1_minerva_ldtk, q2_minerva_ldtk, err_q1_minerva_ldtk, err_q2_minerva_ldtk = models.inverse_limb_darkening(u1_minerva, u2_minerva, err_u1_minerva, err_u2_minerva)

# ---- NGTS (520-890 nm) ----
u1_ngts, u2_ngts = 0.32324499, 0.38972952
err_u1_ngts, err_u2_ngts = 0.00270954, 0.00428052

q1_ngts_ldtk, q2_ngts_ldtk, err_q1_ngts_ldtk, err_q2_ngts_ldtk = models.inverse_limb_darkening(u1_ngts, u2_ngts, err_u1_ngts, err_u2_ngts)

# ---- TRAPPIST (R-band) ----
u1_trap, u2_trap = 0.3421741, 0.38173068
err_u1_trap, err_u2_trap = 0.00264018, 0.00422087

q1_trap_ldtk, q2_trap_ldtk, err_q1_trap_ldtk, err_q2_trap_ldtk = models.inverse_limb_darkening(u1_trap, u2_trap, err_u1_trap, err_u2_trap)
...
#------- GAUSSIAN LIMB DARKENING------------------
def star_log_likelihood_limb_darkening(theta):
    _, _, _, _, mstar, rstar, q1_tess, q2_tess, q1_lco, q2_lco, q1_lcog, q2_lcog, q1_minerva, q2_minerva, q1_ngts, q2_ngts, q1_trap, q2_trap, _, _, _, prot, *_ = theta

    ll_m = -0.5 * ((mstar - mstar_obs) / mstar_obs_err) ** 2
    ll_r = -0.5 * ((rstar - rstar_obs) / rstar_obs_err) ** 2
    ll_prot = -0.5 * ((prot - prot_transit) / prot_transit_err) ** 2

    ll_param = ll_m + ll_r + ll_prot

    ll_ld_tess = (
        -0.5 * ((q1_tess - q1_tess_ldtk) / err_q1_tess_ldtk) ** 2
        -0.5 * ((q2_tess - q2_tess_ldtk) / err_q2_tess_ldtk) ** 2
    )

    ll_ld_lco = (
        -0.5 * ((q1_lco - q1_lco_ldtk) / err_q1_lco_ldtk) ** 2
        -0.5 * ((q2_lco - q2_lco_ldtk) / err_q2_lco_ldtk) ** 2
    )

    ll_ld_lcog = (
        -0.5 * ((q1_lcog - q1_lcog_ldtk) / err_q1_lcog_ldtk) ** 2
        -0.5 * ((q2_lcog - q2_lcog_ldtk) / err_q2_lcog_ldtk) ** 2
    )

    ll_ld_minerva = (
        -0.5 * ((q1_minerva - q1_minerva_ldtk) / err_q1_minerva_ldtk) ** 2
        -0.5 * ((q2_minerva - q2_minerva_ldtk) / err_q2_minerva_ldtk) ** 2
    )

    ll_ld_ngts = (
        -0.5 * ((q1_ngts - q1_ngts_ldtk) / err_q1_ngts_ldtk) ** 2
        -0.5 * ((q2_ngts - q2_ngts_ldtk) / err_q2_ngts_ldtk) ** 2
    )

    ll_ld_trap = (
        -0.5 * ((q1_trap - q1_trap_ldtk) / err_q1_trap_ldtk) ** 2
        -0.5 * ((q2_trap - q2_trap_ldtk) / err_q2_trap_ldtk) ** 2
    )

    ll_LD = ll_ld_tess + ll_ld_lco + ll_ld_lcog + ll_ld_minerva + ll_ld_ngts + ll_ld_trap

    ll_star = ll_param + ll_LD
...
# ============================================================
# Total log-likelihood = transits + stellar + RVs
# ============================================================
def log_likelihood(theta, tess_data, lco_data, lcog_data, minerva_data, ngts_data, trap_data, espresso_data):
    ...
    # stellar log-likelihood assuming uniform prior on limb darkening 
    ll_star = star_log_likelihood(theta) 
    # # stellar log-likelihood assuming GAUSSIAN prior on limb darkening 
    # ll_star = star_log_likelihood_limb_darkening(theta) 
